...
class MeowCommand(base_command.BaseCommand):
    ...
    async def run(self, command_io):
        freq = request.Request("https://some-random-api.ml/facts/cat",
                               headers={"User-Agent": "arbeiterbiene"})
        ireq = request.Request("http://some-random-api.ml/img/cat",
                               headers={"User-Agent": "arbeiterbiene"})
        with request.urlopen(freq) as rfact, request.urlopen(ireq) as rimg:
            ...
            await command_io.message.channel.send(
                fact, embed=Embed().set_image(url=img_url))
